...
def get_3d_point_epipolar(query_ray, pixel_val, context_cam2world, H, W, intrinsics):
    ...
    Args
        query_ray           : [2B, 1024, 6]
        pixel_val           : [2B, N_rays, N_samples, 2]
        context_cam2world   : [2B, 4, 4]

    Return
        p1                  : [2B, N_rays, N_samples, 3]
    """
    ...
    context_plucker = context_plucker.view(b, n_qry, num_epi_points, 6)     # [B, N_rays, N_samples, 6]
...
def get_intersection(line_1, line_2):
    ...
    l1 = line_1[..., :3]    # [2B, 1024, 1, 3]
    l2 = line_2[..., :3]    # [2B, 1024, 1, 3]
    m1 = line_1[..., 3:]    # 
    m2 = line_2[..., 3:]    # 
...
def plucker_embedding(cam2world, uv, intrinsics):
    """
    cam2world   : Extrinsic Parameters
    uv          : Image Coordinate (x, y) 
    """
...
def get_ray_directions_cam(uv, intrinsics, H, W):
    ...
    pixel_points_cam = lift(x_cam, y_cam, z_cam, intrinsics=intrinsics, homogeneous=False)  # [2B, N_rays, N_samples, 3]
    ray_dirs = F.normalize(pixel_points_cam, dim=-1)    # [2B, N_rays, N_samples, 2]
    return ray_dirs
...
def lift(x, y, z, intrinsics, homogeneous=False):
    '''Given (u,v), find Camera Coordi (x, y, z)
...
def project(x, y, z, intrinsics, debug=False):
    '''
    World Coord에서 Camera Coord으로 Pojection했을 때, Pixel Coord 
    z값은 그대로 유지

    Args
        x, y, z         : [B, N_rays, N_samples]
        intrinsics      : [B, 4, 4]
    Return
