...
def encode_relative_point(ray, transform):
    """
    Args
        ray         : [2B, N_rays, N_sample, 3] (c)
        transform   : [B, 2, 4, 4] (c2w)

    Return
        ray         : [2B, N_rays, N_sample, 3] (w)
    """
    s = ray.size()
    b, ncontext = transform.size()[:2]

    ray = ray.view(b, ncontext, *s[1:])                             # [B, 2, N_rays, N_sample, 3]
    ray = torch.cat([ray, torch.ones_like(ray[..., :1])], dim=-1)   # [B, 2, N_rays, N_sample, 4]

    # [B, 2, N_rays, N_sample, 1, 4] * [B, 2, 1, 1, 4, 4] = [B, 2, N_rays, N_sample, 4, 4]
    ray = (ray[:, :, :, :, None, :] * transform[:, :, None, None, :4, :4]).sum(dim=-1)[..., :3] # [B, 2, N_rays, N_sample, 3]
...
class CrossAttentionRenderer(nn.Module):
    def __init__(self, no_sample=False, 
        ...
        if model == "resnet":
            ...
            self.encoder = MultiviewEncoder(num_feat_levels=num_feat_levels, num_queries=num_queries, hidden_dim=hidden_dim, nheads=nheads, num_depth=depth, contra_loss=contra_loss)
    ...
    def forward(self, input, z=None, val=False, debug=False):
        ...
        if z is None:
            if self.model == 'query':
                ...
                z = z_orig = self.get_z(input)  # [2B, 256, H/4, W/4], [2B, 256, H/2, W/2], [2B, 64, H, W] 
        else:
            z_orig = z
        ...
        if self.no_sample:
            ...
            start = start[..., :2]  # [B, 2, N_rays, N_samples, 2] (C)
            end = end[..., :2]      # [B, 2, N_rays, N_samples, 2] (C)
        ...
        if pixel_val is None and (not self.no_sample):
            ...
            pixel_val = torch.flatten(pixel_val, 0, 1)      # [2B, N_rays, N_samples, 2] (I)

        # Gather corresponding features on line
        # [2B, 256+256+64, N_rays, N_samples]
        ...
        if self.n_view == 2 and (not self.no_latent_concat):
            ...
            # context_rel_cam2world_view1 : I, R12
            # context_rel_cam2world_view2 : R21, I
            context_rel_cam2world_view1 = torch.matmul(torch.inverse(context['cam2world'][:, 0:1]), context['cam2world']) # [B, 2, 4, 4]
            context_rel_cam2world_view2 = torch.matmul(torch.inverse(context['cam2world'][:, 1:2]), context['cam2world'])
            
            # pt_view1 : [pt, pt_trans] (W)
            # pt_view2 : [pt_trans, pt] (W)
            pt_view1 = encode_relative_point(pt, context_rel_cam2world_view1) # [2B, N_rays, N_samples, 3] (W)
            pt_view2 = encode_relative_point(pt, context_rel_cam2world_view2) 

            intrinsics_view1 = context['intrinsics'][:, 0]  # [B, 4, 4]
            intrinsics_view2 = context['intrinsics'][:, 1]  # [B, 4, 4]

            s = pt_view1.size()
            pt_view1 = pt_view1.view(b, n_context, *s[1:])  # [B, 2, N_rays, N_samples, 3]
            pt_view2 = pt_view2.view(b, n_context, *s[1:])

            s = interp_val.size()
            interp_val = interp_val.view(b, n_context, *s[1:])

            # Context image feature maps
            interp_val_1 = interp_val[:, 0] # [B, 256+256+64, H]
            interp_val_2 = interp_val[:, 1] #

            # View1에서 본 context2의 좌표
            pt_view1_context1 = pt_view1[:, 0] # = pt (C, W) # [B, 2, N_rays, N_samples, 3]
            pt_view1_context2 = pt_view1[:, 1] # Cam2 Coord

            pt_view2_context1 = pt_view2[:, 0] # Cam1 Coord
            pt_view2_context2 = pt_view2[:, 1] # = pt (C, W)

            # geometry.project          : World -> Camera2 Coordinate -> Image2 Coordinate
            # View2에서 본 context1의 좌표를 view2로 Projection
            # pixel_val_view2_context1  : [B, N_rays, N_samples, 3] (C)
            # pixel_val_view2_context1  : [B, N_rays, N_samples, 2] (I)
            ...
            pixel_val_stack = torch.stack([pixel_val_view1_context2, pixel_val_view2_context1], dim=1).flatten(0, 1)    # [2B, N_rays, N_samples, 2]
            ...
            interp_val_nearest = interp_val_nearest.view(b, n_context, *s[1:])      # [B, 2, 512+64, N_rays, N_samples]
            
            # view1에서 본 context2의 좌표에서 가져온 Feature
            interp_val_nearest_1 = interp_val_nearest[:, 0]
            interp_val_nearest_2 = interp_val_nearest[:, 1]

            pt_view1_context1 = torch.nan_to_num(pt_view1_context1, 0)  # Nan to zero
            ...
            # interp_val_1 : [B, 256+256+64, N_rays, N_samples], pt_view1_context1 : [B, 3, N_rays, N_samples]
            ...
            # 256+256+64+3 - 256+256+64 - (256+256+64)/2
            interp_val_1_encode_1 = self.query_encode_latent_2(F.relu(self.query_encode_latent(interp_val_1_view_1)))   # [B, dim/2, N_rays, N_samples]
            interp_val_1_encode_2 = self.query_encode_latent_2(F.relu(self.query_encode_latent(interp_val_1_view_2)))
            interp_val_1_avg = torch.stack([interp_val_1_encode_1, interp_val_1_encode_2], dim=1).flatten(1, 2)         # [B, 2, dim/2, N_rays, N_samples] -> # [B, dim, N_rays, N_samples]
            ...
            # N_rays, N_samples features
            interp_val = torch.stack([interp_val_1_avg, interp_val_2_avg], dim=1).flatten(0, 1) # [2B, dim, N_rays, N_samples]
        ...
        joint_latent = self.latent_value(interp_val)    # [2B, dim, N_rays, N_samples]
        s = interp_val.size()

        # Compute key value
        # [2B, hidden_dim, N_rays, N_samples]
        ...
        # pixel_val : [2B, N_rays, N_samples, 2] (I)
        # cam_rays  : [2B, N_rays, N_samples, 3] (C) Origin - Sample point 
        ...
        ray_dir = lf_coords[..., :3].flatten(0, 1)  # (B, 2, N_rays, 6) -> [2B, N_rays, 3] 
        ray_dir = ray_dir[:, :, None]               
        ray_dir = ray_dir.expand(-1, -1, cam_rays.size(2), -1) # [2B, N_rays, N_samples, 3]
        ...
        query_ray_orig = geometry.get_ray_origin(query_cam2world).flatten(0, 1) # [2B, 4, 4] - [2B, 3]
        query_ray_orig = query_ray_orig[:, None, None]
        query_ray_orig_ex = torch.broadcast_to(query_ray_orig, cam_rays.size()) # [2B, N_rays, N_samples, 3]

        # Compute depth of the computed 3D coordinate (with respect to query camera)
        depth = torch.norm(pt - query_ray_orig, p=2, dim=-1)[..., None]         # [2B, N_rays, N_samples, 1]
        ...
        # cam_rays, cam_origin, ray_dir, query_ray_orig_ex : [2B, N_rays, N_samples, 3] - 12
        # depth_encode : [2B, N_rays, N_samples, 4]
        local_coords = torch.cat([cam_rays, cam_origin, ray_dir, depth_encode, query_ray_orig_ex], dim=-1).permute(0, 3, 1, 2)  # [2B, N_rays, N_samples, 16]
        coords_embed = self.query_embed_2(F.relu(self.query_embed(local_coords)))

        # Multiply key and value pairs
        dot_at_joint = torch.einsum('bijk,bijk->bjk', key_val, coords_embed) / 16. # [2B, N_rays, N_samples]
        # [B, 2, N_rays, N_samples] - [B, N_rays, 2, N_samples] - [B, N_rays, 2*N_samples]  
        ...
        at_wt_joint = F.softmax(dot_at_joint, dim=-1)       # [B, N_rays, 2*N_samples]
        at_wt_joint = torch.flatten(at_wt_joint.view(b, n_qry_rays, self.n_view, self.npoints).permute(0, 2, 1, 3), 0, 1) # [2B, N_rays, N_samples]

        z_local = (joint_latent * at_wt_joint[:, None, :, :]).sum(dim=-1)       # [2B, dim, N_rays]
        s = z_local.size()                                                      # [2B, dim, N_rays]
        z_local = z_local.view(b, n_context, s[1], n_qry_rays)                  # [B, 2, dim, N_rays]
        z_sum = z_local.sum(dim=1)                                              # [B, dim, N_rays]
        z_local = torch.cat([z_sum for i in range(n_context)], dim=1).view(*s)  # [2B, dim, N_rays]

        at_wt = at_wt_joint # [B, N_rays, 2*N_samples]
        at_wts.append(at_wt)

        # A second round of attention to gather additional information
        if self.repeat_attention:
            z_embed = self.encode_latent(z_local)   # [dim -> 128]
            ...
            query_embed_local = torch.cat([z_embed_local, local_coords], dim=1) # [2B, N_rays, N_samples, 16+128=144]
            ...
            z_local = z_local.view(b, n_context, s[1], n_qry_rays)                  # [B, 2, 144*2, 1024]

            z_sum = z_local.sum(dim=1)
            z_local = torch.cat([z_sum for i in range(n_context)], dim=1).view(*s)  # [2B, 144*2, 1024]

        latents_out.append(z_local)

        z = torch.cat(latents_out, dim=1).permute(0, 2, 1).contiguous() # [2B, 288(144*2), 1024] - [2B, 1024, 288(144*2)]
        out_dict['pixel_val'] = pixel_val.cpu()
        out_dict['at_wts'] = at_wts

        depth_squeeze = depth[..., 0]
        at_max_idx = at_wt[..., :].argmax(dim=-1)[..., None, None].expand(-1, -1, -1, 3) # [B, N_rays, 1, 3]
        ...
        # out_dict['coords'] : [2B, N_rays, 6]
        # query_ray_orig_ex[:, :, 0, :] : [2B, N_rays, 3]
        # out_dict['coords'] : [ray_dir] + [momnet] + [ray_ori]
        out_dict['coords'] = torch.cat([out_dict['coords'], query_ray_orig_ex[:, :, 0, :]], dim=-1) # [2B, N_rays, 9]

        # Plucker embedding for query ray 
        coords = out_dict['coords']
        s = coords.size()
        coords = torch.flatten(coords.view(b, n_context, n_qry_rays, s[-1]).permute(0, 2, 1, 3), -2, -1) # [2, B, 9*N_rays]

        zsize = z.size()    
        z_flat = z.view(b, n_context, *zsize[1:]).permute(0, 2, 1, 3)   # [B, 2, dim, N_rays]
        z_flat = torch.flatten(z_flat, -2, -1)                          # [B, 2, dim*N_rays]

        coords = torch.cat((z_flat, coords), dim=-1)                    # [B, 2, (dim+9)*N_rays]
